    ...
    <div class="row">
      <div class="col-6 offset-3 pt-3 card mt-5 shadow">
        <div class="card-body">
          ...
          <hr>
          <button class="btn btn-primary" :disabled="disabled" @click="saveProduct">Kaydet</button>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    disabled() {
      return !(this.product.title.length > 0 && this.product.count > 0 && this.product.price > 0 && this.product.description.length > 0);
    },
    isLoading(){
  ...
  beforeRouteLeave(to, from, next) {
    if (!this.saveButtonClicked && (this.product.title.length > 0 || this.product.count > 0 || this.product.price > 0 || this.product.description.length > 0)) {
